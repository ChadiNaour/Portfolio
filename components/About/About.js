    ...
    <section ref={AboutRef} className="h-full relative flex z-10  w-full flex flex-col justify-center items-center overflow-hidden bg-dark">
      ...
      <div ref={ref}>
        <motion.div animate={animation} initial="hidden" variants={typeVariants}
          ...
          <div className='hidden  sm:block absolute font-reloadLight  text-white text-lg  md:text-xl pl-6 md:pl-10 z-10 pt-4 -rotate-90' style={{ userSelect: "none" }}>ABOUT</div>
        </motion.div>
      ...
      >ABOUT</div>
      <div className='flex justify-center items-center flex-row md:w-4/5  md:h-2/3  z-20'>
        {/* left container */}
        <div className=' flex  flex-col h-full w-2/3 '>
          <div className="w-full h-full relative flex flex-col items-start justify-center ">
            <div className='font-cinetype text-5xl  mb-4' style={{ color: "transparent", WebkitTextStroke: "0.02em #fff" }}>HI THERE</div>
            <div className='font-cinetype text-4xl  text-white leading-snug tracking-wide pl-1 mb-6'>I'm a passionate Front-end web developer based in casablanca morroco, I specialize in designing and developing clean, functional and interactive user experiences, I'm quietly confident, curious, and perpetually working on improving myself.</div>

            <div className='mr-8'>
              <div className='w-full px-16 h-24 bg-gradient-to-b from-[#312E2E] to-[#403D3D] mb-6 rounded-md flex flex-row gap-24 items-center shadow-md'>
                <div className='flex justify-start items-center  h-full'><span className='text-white font-reloadLight text-2xl'>MAIN STACK</span></div>
                <div className='flex flex-row gap-6 '>
                  <div className='flex justify-end items-center h-20 w-20 bg-green-200 rounded-md bg-[#403D3D] flex justify-center items-center  w-20'><Icon className="w-12 h-12" icon="logos:react" /></div>
                  <div className='flex justify-end items-center h-20 w-20 bg-green-200 rounded-md bg-[#403D3D] flex justify-center items-center  w-20'><Icon className="w-12 h-12 text-white" icon="cib:next-js" /></div>
                  <div className='flex justify-end items-center h-20 w-20 bg-green-200 rounded-md bg-[#403D3D] flex justify-center items-center  w-20'><Icon className="w-12 h-12 rounded-sm" icon="logos:javascript" /></div>
                  <div className='flex justify-end items-center h-20 w-20 bg-green-200 rounded-md bg-[#403D3D] flex justify-center items-center  w-20'><Icon className="w-12 h-12 rounded-sm" icon="logos:typescript-icon" /></div>
                  <div className='flex justify-end items-center h-20 w-20 bg-green-200 rounded-md bg-[#403D3D] flex justify-center items-center  w-20'><Icon className="w-12 h-12" icon="logos:figma" /></div>
                </div>
              </div>
            </div>
            <div className='flex flex-row'>
              <button className={`${AboutStyles.btn} ${AboutStyles.btnCircle}`}
                onClick={(e) => {
                  e.preventDefault();
                  setOpen(!open);
                }}
              >
                <span className='cotet z-20 font-reloadLight mt-1 text-sm lg:text-xl xl:text-xl flex flex-row items-center'>RESUME<svg xmlns="http://www.w3.org/2000/svg" className={`${AboutStyles.arrow}`} viewBox="0 0 448 512"><path d="M438.6 278.6l-160 160C272.4 444.9 264.2 448 256 448s-16.38-3.125-22.62-9.375c-12.5-12.5-12.5-32.75 0-45.25L338.8 288H32C14.33 288 .0016 273.7 .0016 256S14.33 224 32 224h306.8l-105.4-105.4c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0l160 160C451.1 245.9 451.1 266.1 438.6 278.6z" /></svg></span>
              </button>
              <button className={`${AboutStyles.btn2} ${AboutStyles.btnCircle2} ml-6`}
                onClick={(e) => {
                  e.preventDefault();
                  setOpen(!open);
                }}
              >
                <span className='cotet z-20 font-reloadLight mt-1 text-sm lg:text-xl xl:text-xl flex flex-row items-center'>Get in touch<svg xmlns="http://www.w3.org/2000/svg" className={`${AboutStyles.arrow2}`} viewBox="0 0 448 512"><path d="M438.6 278.6l-160 160C272.4 444.9 264.2 448 256 448s-16.38-3.125-22.62-9.375c-12.5-12.5-12.5-32.75 0-45.25L338.8 288H32C14.33 288 .0016 273.7 .0016 256S14.33 224 32 224h306.8l-105.4-105.4c-12.5-12.5-12.5-32.75 0-45.25s32.75-12.5 45.25 0l160 160C451.1 245.9 451.1 266.1 438.6 278.6z" /></svg></span>
              </button>
            </div>
          </div>
        </div>
        {/* right container */}
        <div className=" pb-20 flex flex-col justify-center items-center h-full w-1/3 pl-20">
          <div className='flex flex-col justify-center items-center relative h-[760px] w-full '>
            {/* emoji div */}
            <div className="">
              <Image layout="fixed" width={520} height={520} src={Memoji} />
            </div>
            {/* glass card */}
            <div className='-mt-[60px] bg-white bg-opacity-20 backdrop-blur-sm rounded-lg drop-shadow-lg w-[400px] h-[150px] flex justify-center items-center'>
              ...
              <div className=" flex justify-center items-start   relative">
                <span className='text-xl text-white font-cinetype pb-6'>Do it or dont, there is no try</span>
                <span className="absolute text-md text-white right-0 bottom-0">— YODA</span>
              </div>
